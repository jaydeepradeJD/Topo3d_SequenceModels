...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        data_files = ['F:\\jrade\\data_32\\Data_complaince based\\traindata_SignificantIterations.npz', 'F:\\jrade\\data_32\\Data_complaince based\\valdata_SignificantIterations.npz'] 
        obj = CNN()
        model = obj.gen_model()
        model.summary()
    ...
    if args.mode == 'test':
        ...
        # test_data = DataProcessor('F:\\jrade\\data_32\\Data_complaince based\\testdata_sampled_significantIterations.npz',
        #     batch_size =args.batch_size, train=False)
        test_data = DataProcessor('F:\\jrade\\data_32\\Data_complaince based\\testcases_significantIterations.npz',
            batch_size =args.batch_size, train=False)
        ...
        # for i in range(1): 
            ...
            for i in range(len(output)):
                print('Sample %s.'%i, '  Min Target - ', np.amin(output[i]), '  Max Target - ', np.amax(output[i]))
                print('Sample %s.'%i, '  Min Pred - ', np.amin(predoutputs[i]), '  Max Pred - ', np.amax(predoutputs[i]))
            
            path = './preds/Vis2D'        
            vis2d(input_list, output, predoutputs, k, path=path)
            
            Inouts = np.ones_like(output)
            path = './preds/Vis_3dslices'
            vis3d(input_list[:, :, :, 15:17, :], Inouts[:, :, :, 15:17, :], output[:, :, :, 15:17, :], predoutputs[:, :, :, 15:17, :], k, path=path)
            
            path = './preds/Vis3D'
            vis3d(input_list, Inouts, output, predoutputs, k, path=path)
