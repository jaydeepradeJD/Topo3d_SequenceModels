...
class DataProcessor(keras.utils.Sequence):
    ...
    def __init__(self, filepath, batch_size=64, train = False):
        ...
        self.nextSE = np.expand_dims(self.data['nextSE_smooth'],axis=-1)
        self.nextVD = np.expand_dims(self.data['nextVD_smooth'],axis=-1)
        
        self.batch_size = batch_size
        self.labelled = True   
        self.indexes = np.arange(self.nextVD.shape[0])
    ...
    def _data_process(self, indexes):
        ...
        X3 = np.zeros((len(indexes), 32, 32, 32, 1))
        ...
        for i, index in enumerate(indexes):
            ...
            nSE = self.nextSE[index, :, :, :, 0]
            
            cVD = self.currentVD[index, :, :, :, 0]
            nVD = self.nextVD[index, :, :, :, 0]
            
            X1[i, :, :, :, 0] = action_dict[action](self._calcCompliance(cSE, cVD))
            X2[i, :, :, :, 0] = action_dict[action](cVD)
            vf[i, :, :, :, 0] = np.ones_like(cVD)*np.mean(cVD)

            y[i, :, :, :, 0] = action_dict[action](nVD)
