...
def vis2d(SEinput, VDinput, VDtarget, VDpred, k, path):
    ...
    for i in range(0, vdtarget.shape[0], 2):
        cols = ['{}'.format(name) for name in ['InitialComp  ', '  VD1', '  TargetVD ', ' PredVD']]
...
def VisMPLComparison(Input, Inouts, Target, Predicted, DataId, path='MPLs'):
    ...
    axInput.text2D(0.5, 1.0, "InitialComp", transform=axInput.transAxes)
    ...
    axTarget.text2D(0.5, 1.0, "TargetVD", transform=axTarget.transAxes)
    ...
    axPredicted.text2D(0.5, 1.0, "PredictedVD", transform=axPredicted.transAxes)
